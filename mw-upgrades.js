...
/* ---------- Dashboard-only FABs: modern Settings + Upload chooser ---------- */
if(/dashboard\.html?$/.test(location.pathname) || location.pathname.endsWith('/')){
  window.addEventListener('load', ()=>{
    if(document.getElementById('mw-fab-settings')) return;
    const settings = document.createElement('button');
    settings.id='mw-fab-settings'; settings.className='mw-fab';
    settings.style.cssText += ';bottom:142px;background:linear-gradient(135deg,#334155,#0f172a)';
    settings.innerHTML='⚙️'; settings.dataset.mwSettings='1';
    settings.title='Settings';
    document.body.appendChild(settings);

    const up = document.createElement('button');
    up.id='mw-fab-upload'; up.className='mw-fab';
    up.innerHTML='＋'; up.title='New post / reel';
    up.onclick = async ()=>{
      const choice = await window.MW.chooseUpload();
      if(!choice) return;
      let file=null, kind='post';
      if(choice==='camera-photo') file = await window.MW.captureFromCamera('photo');
      else if(choice==='camera-reel'){ file = await window.MW.captureFromCamera('video'); kind='reel'; }
      else { const inp=document.createElement('input'); inp.type='file'; inp.accept = choice==='gallery-reel'?'video/*':'image/*,video/*'; inp.click(); file = await new Promise(r=>inp.onchange=()=>r(inp.files[0])); kind = choice==='gallery-reel'?'reel':'post'; }
      if(!file) return;
      window.MW.toast('Optimizing media…');
      if(file.type.startsWith('video/')) file = await window.MW.downscaleVideo(file, 720);
      // delegate to existing upload pipeline if present
      if(typeof window.uploadCloud === 'function' && window.firebaseDB && window.currentUser){
        const url = await window.uploadCloud(file);
        ...
        const data = {
          ...
          text: '',
          [isVid?'video':'image']: url,
          type: kind,
          views: window.MW.seedViews(),
          likes: [], comments: [],
          at: serverTimestamp()
        };
        await addDoc(collection(window.firebaseDB, kind==='reel'?'reels':'posts'), data);
        window.MW.toast(kind==='reel'?'Reel posted ✨':'Post shared 🎉','success');
        window.MW.logActivity(window.firebaseDB, window.currentUser, kind==='reel'?'upload_reel':'upload_post');
      } else {
        window.MW.toast('Upload pipeline missing','error');
      }
    };
